...
export default async (req: MedusaRequest, res: Response) => {
      ...
      await Promise.all(
        validated?.items.map(async (i) => {
          ...
          return await cartService
            .withTransaction(manager)
            .addLineItem(cart.id, lineItem as LineItem, {
  ...
  if (req.user && req.user.customer_id) {
    ...
    const user = await userService.retrieve_(req.user.customer_id)
    if (user?.cart_id) {
      const cart = await cartService.retrieve(user?.cart_id)
      if (cart.completed_at) {
        await userService.update_(req.user.customer_id, { cart_id: cart.id })
      }
    } else {
      await userService.update_(req.user.customer_id, { cart_id: cart.id })
    }
