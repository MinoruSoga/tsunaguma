...
/**
 * @oas [get] /list-order-buyer
 * operationId: GetListOrderByBuyer
 * summary: Get list order by buyer
 * parameters:
 ...
 */
const getListOrderBuyer = async (
  req: MedusaAuthenticatedRequest,
  res: Response,
) => {
  const orderService: OrderService = req.scope.resolve('orderService')
  const loggedInUser: LoggedInUser = req.scope.resolve('loggedInUser')
  const [orders, count] = await orderService.listOrdersBuyer(
    loggedInUser.id,
...
export default getListOrderBuyer

export class GetListOrderBuyerParams {
