...
/**
 * @oas [get] /return/history/{id}/cms
 * operationId: "GetDetailReturnHistoryCms"
 * summary: "Get detail return history cms"
 * description: "Retrieves a return history"
 * x-authenticated: true
 * parameters:
 *   - (path) id=* {string} The ID of the Return history.
 * security:
 *   - api_token: []
 *   - cookie_auth: []
 * tags:
...
export default async function (req: MedusaAuthenticatedRequest, res: Response) {
  ...
  const returnHistoryService = req.scope.resolve<ReturnHistoryService>(
    ReturnHistoryService.resolutionKey,
  )

  const { id } = req.params
  const history = await returnHistoryService.getOne(id)
