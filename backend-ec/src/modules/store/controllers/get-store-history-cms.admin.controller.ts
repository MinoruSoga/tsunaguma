...
/**
 * @oas [get] /store/history/{id}/cms
 * operationId: "GetStoreHistoryCms"
 * summary: "Get store history cms"
 * description: "Retrieves a store history"
 * x-authenticated: true
 * parameters:
 *   - (path) id=* {string} The ID of the store history.
 * security:
 *   - api_token: []
 *   - cookie_auth: []
 * tags:
...
export default async function (req: MedusaAuthenticatedRequest, res: Response) {
  ...
  const storeHistoryService = req.scope.resolve<StoreHistoryService>(
    StoreHistoryService.resolutionKey,
  )

  const { id } = req.params
  const history = await storeHistoryService.getOne(id)
